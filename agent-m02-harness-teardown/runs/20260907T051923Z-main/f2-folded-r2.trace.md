...
```
검증 완료 — 예시(`emit([{"a": 1}, {"b": 2}])` → `'{"a": 1}\n{"b": 2}'`)와 빈 목록 케이스, 각 줄 `json.loads` 파싱 모두 실행해 확인함.

PLAN.md S16(D8)에 따라 `src/emit.py`의 `emit`을 NDJSON 직렬화(`json.dumps(row, ensure_ascii=False)`를 `"\n"`으로 연결, 마지막 개행 없음, 빈 목록은 빈 문자열)로 구현했다.
```
