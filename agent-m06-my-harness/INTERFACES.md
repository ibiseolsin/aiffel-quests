...
- 잘못된 입력과 오류 출력 예:
  - `{"path": "../secret.txt", ...}` → `{"ok": false, "error": "path_rejected: 상위 폴더 이동(..)은 허용하지 않는다", "hint": "작업 폴더 기준 상대경로를 쓰세요. 작업 폴더의 실제 경로 후보: receipt.py, test_receipt.py, …"}`
...
|---|---|---|---|---|
| `list_files` | `{}` | `{"files": [...], "count": n, "hidden": [...], "hidden_count": n, "truncated": bool, "start_dir": "…"}` 최대 300개 | 작업 폴더 하위 전체 | 없음. 심링크·`__pycache__` 제외. **숨김 경로는 `hidden` 으로 따로 알려 준다** — 존재는 알리고 열지는 않는다. `files` 가 비고 `hidden` 이 있으면 읽는 방법을 `hint` 로 준다 |
...
- 도구 출력이 16,000자를 넘으면 앞뒤를 남기고 가운데를 `[출력 생략 N자 · 원본 M자]` 로 줄인다.
- **경로 실패(`not_found`·`path_rejected`·`not_python`·`not_a_file`)의 `hint` 에는 하네스가 그 순간
  관측한 실제 후보 경로(최대 5개)와, 숨김 파일이 있으면 그것을 읽는 방법을 함께 담는다.**
- 실행 시작 시 같은 관측을 첫 사용자 메시지에 한 번 넣는다 (`orientation.brief`).
